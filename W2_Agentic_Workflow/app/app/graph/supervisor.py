...
Uses GPT-4o-mini; logs AgentDecision. No keyword heuristic fallbacks.
"""
import json
import logging
import time
...
def supervisor_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not settings.has_openai:
        decision = {
            ...
            "reasoning": "No OpenAI key configured — cannot classify intent. Defaulting to plan.",
    ...
    try:
        ...
        reasoning = f"LLM classification failed ({str(e)[:80]}). Defaulting to plan intent."
        decision = {
            "agent_name": "supervisor",
            "action": "classify_intent",
            "reasoning": reasoning,
